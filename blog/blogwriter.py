...
class HandWriter(MDApp):
    ...
    def texttohand(self):
        ...
        if len(fname) != 0 and len(txt) !=0:
            self.loader.open()
            title = f'{fname}'
            data = {"title":title,"content":txt}
            image = {"image":open("hanuman.jpg","rb")}
            r = requests.post("http://192.168.1.97:5000/add",data=data,files=image,auth=("root","chakrahar"))
            self.rloader = MDDialog(
            title = "Response",
            text=str(r.text)
            )
            self.rloader.open()
